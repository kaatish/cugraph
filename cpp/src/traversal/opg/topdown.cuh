...
{
  // BlockScan
  typedef cub::BlockScan<edge_t, TOP_DOWN_EXPAND_DIMX> BlockScan;
  __shared__ typename BlockScan::TempStorage scan_storage;

  // We will do a scan to know where to write in frontier
  // This will contain the common offset of the block
  __shared__ edge_t frontier_common_block_offset;

  __shared__ edge_t shared_buckets_offsets[TOP_DOWN_EXPAND_DIMX - NBUCKETS_PER_BLOCK + 1];
  __shared__ edge_t shared_frontier_degrees_exclusive_sum[TOP_DOWN_EXPAND_DIMX + 1];
  ...
  __shared__ vertex_t block_n_frontier_candidates;

  edge_t block_offset = (blockDim.x * blockIdx.x) * max_items_per_thread;

  constexpr vertex_t invalid_vid = cugraph::invalid_idx<vertex_t>::value;
  constexpr edge_t invalid_eid = cugraph::invalid_idx<edge_t>::value;

  edge_t n_items_per_thread_left =
  ...
  for (; (n_items_per_thread_left > 0) && (block_offset < totaldegree);
       ...
       n_items_per_thread_left -= min(
         n_items_per_thread_left, static_cast<edge_t>(MAX_ITEMS_PER_THREAD_PER_OFFSETS_LOAD))) {
    // In this loop, we will process batch_set_size batches
    edge_t nitems_per_thread =
      min(n_items_per_thread_left, static_cast<edge_t>(MAX_ITEMS_PER_THREAD_PER_OFFSETS_LOAD));
    ...
    edge_t left  = 0;
    edge_t right = nitems_per_thread;
    while (left < nitems_per_thread) {
      ...
      edge_t nvalues_to_load = shared_buckets_offsets[right * NBUCKETS_PER_BLOCK] -
      ...
      edge_t nitems_per_thread_for_this_load = right - left;

      edge_t frontier_degrees_exclusive_sum_block_offset =
      ...
      for (edge_t item_index = 0; item_index < nitems_per_thread_for_this_load;
        ...
        vertex_t vec_u[TOP_DOWN_BATCH_SIZE];
        vertex_t vec_v[TOP_DOWN_BATCH_SIZE];
        edge_t local_buf1[TOP_DOWN_BATCH_SIZE];
        edge_t local_buf2[TOP_DOWN_BATCH_SIZE];
        unsigned vec_v_visited_bmap[TOP_DOWN_BATCH_SIZE];

        edge_t *vec_frontier_degrees_exclusive_sum_index = &local_buf2[0];

#pragma unroll
        for (edge_t iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          edge_t ibatch = left + item_index + iv;
          edge_t gid    = block_offset + ibatch * blockDim.x + threadIdx.x;

          if (gid < current_max_edge_index) {
            edge_t start_off_idx = (ibatch * blockDim.x + threadIdx.x) / TOP_DOWN_BUCKET_SIZE;
            edge_t bucket_start =
              shared_buckets_offsets[start_off_idx] - frontier_degrees_exclusive_sum_block_offset;
            edge_t bucket_end = shared_buckets_offsets[start_off_idx + 1] -
                                   frontier_degrees_exclusive_sum_block_offset;

            edge_t k = cugraph::detail::traversal::binsearch_maxle<edge_t>(
            ...
            vec_u[iv]                                    = invalid_vid;
            vec_frontier_degrees_exclusive_sum_index[iv] = invalid_eid;
          }
        }

        edge_t *vec_row_ptr_u = &local_buf1[0];
#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          vertex_t u = vec_u[iv];
          // row_ptr for this vertex origin u
          vec_row_ptr_u[iv] = (u != invalid_vid) ? offsets[u - vertex_begin] : invalid_eid;
        }

#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          edge_t thread_item_index = left + item_index + iv;
          edge_t gid               = block_offset + thread_item_index * blockDim.x + threadIdx.x;

          edge_t row_ptr_u = vec_row_ptr_u[iv];
          // Need this check so that we don't use invalid values of edge to index
          if (row_ptr_u != invalid_eid) {
            edge_t edge = row_ptr_u + gid - vec_frontier_degrees_exclusive_sum_index[iv];

            // Destination of this edge
            vec_v[iv] = indices[edge];
          }
        }
...
#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          ...
          vec_v_visited_bmap[iv] =
            (v != invalid_vid) ? bmap[v / INT_SIZE] : (~unsigned(0));  // will look visited
        }
        ...
        vertex_t *vec_frontier_candidate = vec_v;

#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          vertex_t v = vec_frontier_candidate[iv];
          unsigned m       = 1 << (v % INT_SIZE);

          unsigned is_visited = vec_v_visited_bmap[iv] & m;

          if (is_visited) vec_frontier_candidate[iv] = invalid_vid;
        }

        // vec_v_visited_bmap is available
        unsigned *vec_is_isolated_bmap = vec_v_visited_bmap;

#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          vertex_t v              = vec_frontier_candidate[iv];
          vec_is_isolated_bmap[iv] = (v != invalid_vid) ? isolated_bmap[v / INT_SIZE] : ~unsigned(0);
        }
...
#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          vertex_t v     = vec_frontier_candidate[iv];
          unsigned m           = 1 << (v % INT_SIZE);
          unsigned is_isolated = vec_is_isolated_bmap[iv] & m;
          ...
          if (is_isolated && v != invalid_vid) {
            unsigned m = 1 << (v % INT_SIZE);
            atomicOr(&bmap[v / INT_SIZE], m);
            if (distances) distances[v] = level;

            if (predecessors) predecessors[v] = vec_u[iv];

            // This is no longer a candidate, neutralize it
            vec_frontier_candidate[iv] = invalid_vid;
          }
        }

        // Number of successor candidate hold by this thread
        edge_t thread_n_frontier_candidates = 0;

#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          vertex_t v = vec_frontier_candidate[iv];
          if (v != invalid_vid) ++thread_n_frontier_candidates;
        }
        ...
        // Computing offsets
        edge_t thread_frontier_candidate_offset = 0;  // offset inside block
...
#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          ...
          vertex_t frontier_candidate = vec_frontier_candidate[iv];

          if (frontier_candidate != invalid_vid) {
        ...
        edge_t naccepted_vertices = 0;
        // We won't need vec_frontier_candidate after that
        vertex_t *vec_frontier_accepted_vertex = vec_frontier_candidate;

#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          const int idx_shared             = iv * blockDim.x + threadIdx.x;
          vec_frontier_accepted_vertex[iv] = invalid_vid;

          if (idx_shared < block_n_frontier_candidates) {
            vertex_t v = shared_local_new_frontier_candidates[idx_shared];  // popping
                                                                             // queue
            unsigned m = 1 << (v % INT_SIZE);
            unsigned q = atomicOr(&bmap[v / INT_SIZE], m);  // atomicOr returns old

            if (!(m & q)) {  // if this thread was the first to discover this node
              if (distances) distances[v] = level;

              if (predecessors) {
                vertex_t pred  = shared_local_new_frontier_predecessors[idx_shared];
                predecessors[v] = pred;
              }
...
#pragma unroll
        for (int iv = 0; iv < TOP_DOWN_BATCH_SIZE; ++iv) {
          const int idx_shared = iv * blockDim.x + threadIdx.x;
          if (idx_shared < block_n_frontier_candidates) {
            vertex_t new_frontier_vertex = vec_frontier_accepted_vertex[iv];

            if (new_frontier_vertex != invalid_vid) {
              edge_t off     = frontier_common_block_offset + thread_new_frontier_offset++;
